...
`tiller ambient disable` creates `.tiller/ambient.disabled` in the current project. While that marker exists, ambient hooks pass through silently. `tiller ambient enable` removes it. `tiller ambient status` reports the marker and, inside a run, points to the current `status.md` and next action. `tiller ambient next` prints a compact run-control digest from scratch state. `tiller ambient doctor` verifies the live ambient runtime, source/binary drift, bypass state, classifier allowlist, and hook policy smoke behavior. `TILLER_AMBIENT_DISABLED=1` is also honored as a process-local bypass.
