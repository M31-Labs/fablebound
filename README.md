...
Ambient run directories include a generated `status.md` snapshot beside `ledger.jsonl` when lifecycle or usage ledger events are observed. It is derived from `manifest.json`, dispatch metadata, agent lifecycle records, checkpoint candidates, and the ledger so `tiller-summary` can read compact run state before opening raw records.
