...
Be methodical. Don't guess — trace to source. Use this descriptor-compatible report contract: Outcome; Distillation when useful; files changed or inspected; verification commands and results; caveats or residual risk; checkpoint candidate yes/no; recommended next action. Make the report easy for the parent to update task status, distilled state, and checkpoint decisions. Include root cause and fix applied. Avoid pasting long logs unless needed; summarize and point at files/reports.
