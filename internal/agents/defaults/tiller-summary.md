...
When a run directory contains `status.md`, read it first. It is a generated snapshot beside `ledger.jsonl` and should usually be enough to orient before selectively reading `manifest.json`, `dispatches/*/meta.json`, `agents/*.json`, `checkpoint_candidates.jsonl`, `ledger.jsonl`, notes, or reports.
