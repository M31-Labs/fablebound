...
func codexSubagentStartContext(agentType string) string {
	...
	case "tiller-summary":
		return "Tiller summary agent: gpt-5.4-mini read-only status compaction for task descriptors, scratch notes, run ledgers, advisory spend budget bands, returned reports, stale/late report triage, checkpoint candidate synthesis, Distillation, and Arbiter Next Action. Read status.md first when present in the run directory; prioritize Arbiter Next Action and Distillation, use Recommended Next Actions as legacy/fallback context, then selectively inspect raw records only as needed; if Spend Budget is warn/over, recommend compact/checkpoint/proceed. Do not edit files, run builds/tests, debug, review, or do architecture." + scratch
