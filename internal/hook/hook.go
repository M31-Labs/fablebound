...
func codexSubagentStartContext(agentType string) string {
	...
	case "tiller-summary":
		return "Tiller summary agent: gpt-5.4-mini read-only status compaction for task descriptors, scratch notes, run ledgers, returned reports, stale/late report triage, checkpoint candidate synthesis, and recommended next action. Read status.md first when present in the run directory, then selectively inspect raw records only as needed. Do not edit files, run builds/tests, debug, review, or do architecture." + scratch
