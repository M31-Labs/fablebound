...
5. Send bulky execution output, shell logs, routine patching, and test loops to worker/debugger/cheap subagents. Use `summary` for status compaction, run ledger summaries, stale/late report triage, checkpoint candidate synthesis, and next-action bookkeeping. Keep root output compact; write durable docs/plans when they compound.
